"""
Defines the main() function which starts the game, along with
the Game class which is responsible for controlling the flow of the game.
"""
...
class GuessTheNumberGame:
    ...
    def play_again(self):
        ...
        while True:
            ...
            if play_again.lower() in ["y", "n"]:
                ...
                print(
                    f"{Fore.BLUE}Invalid input, please answer with"
                    f"'y' or 'n'.{Style.RESET_ALL}\n"
                    )
    ...
    def select_difficulty(self):
        ...
        while valid is False:
            ...
            if difficulty in ["1", "2", "3"]:
                ...
                print(
                    f"{Fore.BLUE}Invalid input, please select one"
                    f" of the options.{Style.RESET_ALL}\n")
    ...
    def __init__(self):
        ...
        The `leaderboard_file` attribute is set to the filename where
        the leaderboard data is stored.
        The `select_difficulty` method is called to let the player
        select the difficulty level and set the range of numbers to
        be used in the game.
        """
    ...
    def check_guess(self, guess):
        ...
        if guess < self.start or guess > self.end:
            return (False,
                    f"{Fore.BLUE}Your guess is out of bounds."
                    f"Please try again.{Style.RESET_ALL}")
        elif guess < self.target:
            return (True,
                    f"{Fore.YELLOW}Too low! Please try again."
                    f"{Style.RESET_ALL}")
        elif guess > self.target:
            return (True,
                    f"{Fore.YELLOW}Too high! Please try again."
                    f"{Style.RESET_ALL}")
        else:
            return (True,
                    f"{Fore.GREEN}Congratulations! You've"
                    f" guessed the number!{Style.RESET_ALL}")
    ...
    def play(self):
        ...
        while True:
            try:
                ...
                # If the guess is valid (in bounds), append it to the
                # list of guesses
                if valid:
                ...
                if "Congratulations! You've guessed the number!" in result:
                    ...
                    print(
                        f"You made {len(self.guesses)} guesses"
                        f" in this game.\n"
                        )
                    if "Congratulations! You've guessed the number!" in result:
            ...
            # Handle the case where the user's input can't be
            # converted to an integer
            except ValueError:
                print(
                    f"{Fore.BLUE}Invalid input. Please enter a number."
                    f"{Style.RESET_ALL}")
        ...
        # After the game has ended and we're out of the loop, ask if
    ...
    def update_leaderboard(self, score):
        ...
        Try opening the leaderboard file and loading the contents
        into a dictionary
        """
        try:
            with open(self.leaderboard_file, 'r') as file:
                leaderboard = json.load(file)
        # If file not found or content is not valid JSON, create a
        # new leaderboard with infinite scores for each difficulty
        ...
        # Identify the difficulty of the current game based
        # on the start and end numbers
        if self.start == 1 and self.end == 20:
            difficulty = "easy"
        elif self.start == 1 and self.end == 100:
            difficulty = "medium"
        elif self.start == -1000 and self.end == -1:
            difficulty = "hard"
        ...
        # Call the show_best_score method with the updated leaderboard
